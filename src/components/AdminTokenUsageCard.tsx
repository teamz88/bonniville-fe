...
const AdminTokenUsageCard: React.FC<AdminTokenUsageCardProps> = ({ className = '' }) => {
  ...
  const totalTokens = tokenUsageData?.data?.reduce((sum, user) => sum + user.total_tokens_used, 0) || 0;
  const avgTokensPerMessage = tokenUsageData?.data?.reduce((sum, user) => sum + user.avg_tokens_per_message, 0) / (tokenUsageData?.data?.length || 1) || 0;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-900">
                {isLoading ? '...' : formatNumber(Math.round(avgTokensPerMessage))}
              </p>
            </div>
                  ...
                  {topUsers.map((user, index) => (
                    <tr key={user.user_id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {formatNumber(user.input_tokens_used)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {formatNumber(user.output_tokens_used)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
